...
const MyPage = () => {
  ...
  const GetUserFunc = async (e) => {
    axios
      .get("https://wink.kookm.in/user-info", { withCredentials: true })
      .then((response) => {
  ...
  const RemoveUserFunc = async (e) => {
    try {
      let res = await axios.delete(`https://wink.kookm.in/delete/${userId}`, {
        withCredentials: true,
      });
              ...
              >
                <Form.Item>
                  <Button
                    ...
                    style={{ width: "100%" }}
                  >
                    <Link to="/pages/EditInfo">회원정보 수정</Link>
                  </Button>
                </Form.Item>
                <Form.Item>
                  <Button
