...
const ArticleDetail = () => {
    ...
    const getUserId = async () => {
        let res = await axios.get('https://wink.kookm.in/user-info', {withCredentials: true});
        return res.data.userId;
    }


    const GetArticle = async () => {
        const res = await axios.get(`https://wink.kookm.in/communityList/all/${await getUserId()}`, {
    ...
    return (
        <Layout style={{ minHeight: "100vh" }}>
            <Content style={{ padding: "0 20px", flex: 1 }}>
                ...
                >
                    <Link to="/pages/ArticleList" style={{
                        textDecoration : 'none',
                        color : 'black'
                    }}>
                        <LeftOutlined
                    ...
                    <Row justify="center">
                        <Col xs={24} sm={16} md={12} lg={8} xl={6} style={{display:'flex',flexDirection:'column',alignItems:'center', marginTop : '50px'}}>
                            <div style={{
                                width: '90vw',
                                height: '50vh',
                                lineHeight: '1.5',
                                ...
                                <span style={{width: '90%', border: '1px solid black'}}></span>
                                <div style={{width: '90%', height: '90%', display: 'flex', marginTop : '20px'}}>
                                    {article?.content ?? ""}
                                </div>
                            </div>
                        </Col>
